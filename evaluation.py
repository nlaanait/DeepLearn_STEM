...
tf.app.flags.DEFINE_integer('NUM_EXAMPLES_PER_EPOCH', 10000,
                            """Number of classes in training/evaluation data.""")
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      while step < num_iter and not coord.should_stop():
        ...
        print(predictions)
        #print('Predictions: %s' %(format(predictions)))
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
      ...
      while True:
        eval_once(saver, summary_writer, tf.cast(top_k_op,tf.float32), summary_op)
        if FLAGS.run_once:
            break
